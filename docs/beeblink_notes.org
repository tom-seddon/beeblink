...
* Dependencies spring clean

** DONE npm warnings

#+begin_example
  c:\tom\github\beeblink\server>npm install
  ...
  added 383 packages, and audited 384 packages in 33s
...
Fixed by bumping eslint to ^9.0.0.

** TODO eslint flat config stuff

Inevitable warning for now:

#+begin_example
  (node:2740) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///C:/tom/github/beeblink/server/eslint.config.js?mtime=1759427782011 is not specified and it doesn't parse as CommonJS.
  Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
  To eliminate this warning, add "type": "module" to C:\tom\github\beeblink\server\package.json.
...
#+begin_src js
    ...
    "scripts": {
      ...
      "compile": "tsc -p ./tsconfig.json --forceConsistentCasingInFileNames",
      "lint": "eslint -f unix --ext .ts .",
...
Needs adding to package.json. But there's enough fallout from doing
this that it needs to be its own thing.

After attempt 2: looks like serialport just doesn't like this? To be
continued.
...
Possibly useful for the FS stuff too?

** TODO dynamic =require= in main.ts

Can this be sorted out now?

** DONE =void= not valid as constituent in union type
CLOSED: [2025-10-02 Thu 23:04]
...
#+begin_src js
  {
  	...
  	"tasks": [
  		...
  		},
  		{
  			...
  			"label": "npm: compile",
  			"detail": "tsc -p ./tsconfig.json --forceConsistentCasingInFileNames",
  			"presentation": {
  				"clear": true
  			}
  		}
...
#+begin_src yaml
  ...
  build_script:
    ...
    - ps: cd .build
    - ps: 7z a $env:ZIP beeblink_avr_fe60.rom beeblink_tube_serial.rom beeblink_tube_serial_safe.rom beeblink_upurs_fe60.rom beeblink_tube_serial_electron.rom beeblink_tube_serial_safe_electron.rom beeblink_b2.rom
    - ps: Pop-Location
    - ps: 7z a $env:ZIP volumes COPYING.txt
    - ps: Push-Location
    - ps: cd ./server
    - npm install
    - npm run compile
...
#+begin_example
./run.sh --serial-exclude /dev/tty.usbserial-FT4AHAA3
#+end_example

Get bus snoop data:

#+begin_example
cat /dev/cu.usbserial-FT4AHAA3 > ~/tmp/test.dat
#+end_example

Suitable decode6502 invocation:

#+begin_example
./decode6502 --machine=beeb --byte --hex-prefix='$' --state --hex ~/tmp/test.dat
#+end_example

https://apple.stackexchange.com/questions/177448/stty-device-baud-rate-resets-once-no-longer-being-used

https://ftdichip.com/drivers/vcp-drivers/

UPURS thread: https://www.stardot.org.uk/forums/viewtopic.php?t=3114

Comment from Chris: https://stardot.org.uk/forums/viewtopic.php?p=203002#p203002

** Test 1

For testing the basic UPURS transmission, can use the standard UPURS
ROM. Do =*UPLOAD 5800= in Mode 5 to receive data to $5800, then send
it an image file. For example:

#+begin_example
./run.sh --serial-test-send-file ~/github/beeb-files/stuff/Ghouls-Superior/0/\$.GOSTSCR
#+end_example

This works fine in Linux.

This makes a big mess in macOS. Though, interestingly, sending it the
same file via [[https://freeware.the-meiers.org/][CoolTerm]] seems to work ok...

** =tools/send_serial_file.py=

tools/send_serial_file.py works from Linux. Similar results as the
BeebLink server on macOS (possibly identical, but didn't check -
anyway, it doesn't work).

#+begin_example
./send_serial_file.py -i ~/github/beeb-files/stuff/Ghouls-Superior/0/\$.GOSTSCR /dev/tty.usbserial-A97DYC34
#+end_example

Had to open the port in =ab= mode on macOS (for reasons unknown). With
this, some corruption when sending the file from Linux. Seems ok if
opening the port in =wb= mode. =w+b= needs =buffering\=0=, but that
seems to work ok too?

** =tools/send_serial_file/send_serial_file.c=

#+begin_example
  ~/github/beeblink/tools/send_serial_file/send_serial_file -v -i ~/github/beeb-files/stuff/Ghouls-Superior/0/\$.GOSTSCR /dev/cu.usbserial-A97DYC34
#+end_example

Seems to work ok polling for CTS on macOS, which it does automatically
if detecting that the RTS/CTS flag didn't get set. Speed doesn't look
amazing doing this, but that's just going by eye.

Results for -n 1:

: sent 10240 bytes in 6.421 seconds: ~1.557 KBytes/sec

Results for -n 2:

: sent 10240 bytes in 3.208 seconds: ~3.117 KBytes/sec; 25535.3 bits/sec

Results for -n 3 (bad transfer):

: sent 10240 bytes in 2.203 seconds: ~4.539 KBytes/sec; 37181.6 bits/sec

Performance in Linux is terrible if polling CTS, and it's no better if
using TIOCMIWAIT or setting ASYNC_LOW_LATENCY. (But CTS flow control
seems to work fine, so hopefully this case is irrelevant.)

=MDMBUF= doesn't seem to do anything useful.

** USB serial adapters tried
 
Note: Tube Serial device is 0403:6014 Future Technology Devices
...
All tests  performed with this one.

*** 1a86:7523 QinHeng Electronics CH340 serial converter

Tried with Linux. Gets stuck on =Purging link...= on the BBC when
doing =*UPLOAD=, and no obvious way to get it out of this state.
...
#+begin_example
  ...
  Supported CPU types:
      6502      aliases: R6502 SY6502 NMOS 02
      65C02     aliases: CMOS C02
      R65C02    aliases: ROCKWELL
      WD65C02   aliases: W65C02 WDC
      ARLET     aliases:
      ARLETC02  aliases:
      ALANDC02  aliases:
      65816     aliases: 65C816 W65816 W65C816 816 C816
      6800      aliases: M6800 MC6800 6802 M6802 MC6802

   General options:
        --bbctube              BBC tube protocol decoding
    -b, --byte                 Enable byte-wide sample mode
    -c, --cpu=CPU              Sets CPU type (see above)
    -d, --debug=LEVEL          Sets the debug level (0 or 1)
        --labels=FILE          Swift format label/symbols file e.g. from beebasm
        --mem[=HEX]            Memory modelling (see above)
    -m, --machine=MACHINE      Sets machine specific defaults and memory model
                               (see above)
    -p, --profile[=PARAMS]     Profile code execution
        --skew[=SKEW]          Skew the data bus by +/- n samples
        --skew_rd[=SKEW]       Skew the data bus by +/- n samples for read data
        --skew_wr[=SKEW]       Skew the data bus by +/- n samples for write data
   ...
   Signal defintion options:
        ...
        --user[=BITNUM]        Bit number for user (default -1)
...
** dtrace output from CoolTerm

CoolTerm works (or seems to) - what is it doing?!

Open port:

: open("/dev/cu.usbserial-A97DYC34\0", 0x20006, 0x0)		 = 4 0

0x20006 = O_RDWR|O_NONBLOCK|O_NOCTTY

Unique ioctls on the port's fd:

|      ioctl | name     | macro                |     # |
|------------+----------+----------------------+-------|
| 0x2000740D | TIOCEXCL |                      |     1 |
| 0x2000740E | TIOCNXCL |                      |     1 |
| 0x4004667F | FIONREAD |                      |  3047 |
| 0x4004746A | TIOCMGET | _IOR('t', 0x6a, int) | 17602 |
| 0x40487413 | TIOCGETA |                      |     5 |
| 0x8004746D | TIOCMSET | _IOW('t', 0x6d, int) |     4 |
| 0x80487414 | TIOCSETA |                      |     5 |
...
: 0xABCDEFGH
:   ||||||++-- n (the number in the headers)
:   ||||++---- t (the char in the headers)
:   ++++------ len
:   +--------- void/out/in

** More nonsense

After poking about with dtrace - CoolTerm polls for CTS, but it seems
like it's never actually false?!

If send_serial_file.c carefully polls for CTS, it is also always
false. And yet, the transfer seems to work?! - which it doesn't if it
doesn't poll. I'm not sure this isn't coincidence.

If it sends more than 2-3 bytes at a time after polling for CTS=1, the
data doesn't transfer correctly. But dtrace suggests CoolTerm sends
256 bytes at a time? So is this possible then or what?!

TIOCMGET does seem to correctly report the CTS state (e.g., it does
end up reporting CTS=0 after a =*UPLOAD= transfer is complete). So
maybe there's something that could be done.

* Basic BBC-side tests

1. Ensure supplied volumes are available to BeebLink:

   - =beeblink_dfs_tests=
   - =beeblink_tubehost_tests= (a TubeHost volume)

2. Ensure [[https://github.com/tom-seddon/beeb-fstest/][beeb-fstest]] volume is available to BeebLink:

   - =beeb-fstest=

3. Do =*VOL beeblink_dfs_tests=

4. Do SHIFT+BREAK

The tests will fail with an error at some point if anything goes
wrong, or (eventually) print `TESTS SUCCEEDED` if successful.
