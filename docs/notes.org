...
On the server, in response to OSBGET, send more than one (how many?)
OSBGET responses, possibly including a terminating OSBGET_EOF,
covering the byte requested and the bytes coming up. The client will
consume one and the rest will remain in the FIFO.

On the client, when doing OSBGET, if the last request was OSBGET for
the requested handle, check the FIFO. If it's empty, send an OSBGET
request, note the handle, and await response. Otherwise, check the
buffered-up message. If it's OSBGET/OSBGET_EOF, it's the response,
immediately! If it's anything else, that's a protocol error.
...
checking that it's OSBGET/OSBGET_EOF messages that are being received.
Anything else encountered is a protocol error.

Status:

- HTTP: not yet implemented - protocol will need extending as
  currently the BeebLink message payload length is inferred from the
  HTTP message payload length
- Tube Serial: not yet implemented
- UPURS: might actually be possible, as there's a smallish Beeb-side
  recv buffer? So, with a bit of care...
