...
during normal transmission. The server and BBC can just send this
sequence to one another repeatedly until both sides have detected it,
to ensure all buffers are flushed and everything is synced up.
...
just on its own, there's never any payload, and the server just
ignores it.
...
1. BBC and server emit a constant stream of 0 bytes

2. Both sides keep count of runs of zeros seen, and discard anything
   else (presumably just buffered-up junk). Once both have seen 259
   consecutive zeros (which never occurs during ordinary
   transmission), they switch to sync part 2

Sync part 2:

3. Both sides send a single non-0 byte

4. Both sides keep reading until they get the non-0 byte
...
bytes. The PC will always send a full request, and the data is always
valid, so the confirmation bytes are never 0.

** confirmation bytes

Payloads include confirmation bytes at particular points. The
confirmation bytes indicate whether there's more data to come (value
1), or that the payload has been canceled (0 - i.e.,
=REQUEST_SERIAL_CANCEL=).
...
The format for responses is the same. Responses are never canceled,
but confirmation bytes are still included, to ensure there's never a
long enough run of 0s.

Confirmation bytes should also be included mid-payload based on each
byte's negative offset: =(payload size)-(offset of byte)=. (The last
byte's negative offset is =-1=, and the first byte's is =-(payload
size)=.) If the LSB of the negative offset is 0, send a confirmation
byte before sending the byte.

The payload is also always followed by a confirmation byte.
